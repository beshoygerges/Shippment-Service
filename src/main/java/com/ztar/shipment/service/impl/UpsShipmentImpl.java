...
@Service
@Qualifier("ups")
public class UpsShipmentImpl implements ShipmentService {
    ...
    public BigDecimal calculateFees(ShipmentRequest request) {
        ...
        if (getWeightUnit() != POUND) {
            weight = (int) Math.ceil(UnitConverter.convert(weight, getWeightUnit(), POUND));
        }

        if (weight > dimensionalWeight)
            return BigDecimal.valueOf(weight / 10).multiply(getPricePerPound());
